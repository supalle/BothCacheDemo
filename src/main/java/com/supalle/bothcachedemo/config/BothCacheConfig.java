package com.supalle.bothcachedemo.config;
...
/**
 * Please forgive me for not writing java doc
 *
 * @author supalle
 * @date 2021/7/13
 */
...
public class BothCacheConfig {
    private static final Logger LOG = LoggerFactory.getLogger(BothCacheConfig.class);
            ...
            if (valueWrapper == null) {
                synchronized (redisCache) {
                    ...
                    if (valueWrapper == null) {
                        ...
                        if (valueWrapper != null) {
                            LOG.info("cache key {} use redis cache", key);
                            caffeineCache.put(key, valueWrapper.get());
                        }
                    } else {
                        LOG.info("cache key {} use local cache", key);
                    }
                }
            } else {
                LOG.info("cache key {} use local cache", key);
